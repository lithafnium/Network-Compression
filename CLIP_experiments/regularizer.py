...
def search_u_v(conf, model, cache):
    ...
    if conf.reg_update == "adverserial_update":
        u = u.float().requires_grad_()
        v = v.float().requires_grad_()
...
def lip_constant(conf,model, u, v, mean=False):
    uv = torch.cat((u.to(conf.device), v.to(conf.device)), 0)
    num = uv.shape[0] // 2
    output = model(uv.to(conf.device).float())
    u_out = output[:num]
...
def u_v_init(conf, reg_loader, cache):
    ...
    if conf.reg_init == "partial_random":
        init[num:] = X_reg[num:] + torch.rand_like(X_reg, dtype=torch.float)[num:] * 1e-1
        init[:num] = X_reg[num:]
    elif conf.reg_init == "noise":
        init = torch.rand_like(X_reg, dtype=torch.float) * 1e-1
