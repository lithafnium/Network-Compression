...
class Discriminator(nn.Module):
    def __init__(self, ngpu):
        ...
        self.main = nn.Sequential(
            ...
            # nn.Conv2d(ndf, ndf * 16, 4, 2, 1, bias=False),
        ...
        # # state size. (ndf*2) x 16 x 16
        self.c4 = nn.Conv2d(ndf * 4, 1, 62, 1, 0, bias=False)
        self.s = nn.Sigmoid()
    ...
    def forward(self, input):
        ...
        x = self.bn2(x) 
        x = self.r3(x)
        print("cnn 3 output size: ", x.size())
        x = self.c4(x)
        print("cnn 4 output size: ", x.size())
        x = self.s(x) 
        return x
